...
import type {
  GoogleTranslateApiErrorBody,
  GoogleTranslateApiResponse,
  GoogleTranslateRequestBody,
} from '../types/googleTranslate.types.js';
...
export const googleTranslateClient = {
  async translateText(
    ...
    const body: GoogleTranslateRequestBody = {
      q: text,
      source: sourceLang,
      target: targetLang,
      format: 'text',
    ...
    try {
      const { data } = await axios.post<GoogleTranslateApiResponse>(
      ...
      if (isAxiosError<GoogleTranslateApiErrorBody>(error)) {
        ...
        throw new AppError(
          safeStatus,
          apiMessage ?? 'Translation request failed',
          error.response?.data,
          'TRANSLATE_API_ERROR',
        );
      }
